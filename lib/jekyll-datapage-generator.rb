...
module Jekyll
  ...
  class DataPage < Page
    ...
    # - `dir_expr` is an expression for generating output directories
    ...
    def initialize(site, base, index_files, dir, dir_expr, page_data_prefix, data, name, name_expr, title, title_expr, template, extension, debug)
      ...
      if dir_expr
        ...
        dir = sanitize_dirname(raw_dir).to_s
        puts "debug (datapage-gen). using dir_expr: '#{dir}' will be used for the directory name" if debug
      end
